...
def get_weather(city: str):
    ...
    emoji_now, desc  = WEATHER_CODES.get(current["weathercode"], ("🌡️", "알 수 없음"))
    precip_now       = current.get("precipitation_probability", 0)
    temp_max         = round(daily["temperature_2m_max"][0])
    temp_min         = round(daily["temperature_2m_min"][0])
    outfit           = next((o for thr, o in OUTFIT_GUIDE if temp_max >= thr), "🧥 패딩, 두꺼운 코트")

    weekly = []
    for i in range(7):
        dt    = datetime.strptime(daily["time"][i], "%Y-%m-%d")
        day   = WEEKDAYS[dt.weekday()]
        emoji, _ = WEATHER_CODES.get(daily["weathercode"][i], ("🌡️", ""))
        weekly.append((
            day, emoji,
